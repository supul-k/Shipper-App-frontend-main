...
import { Radio, Input, TextArea, Form, Dropdown } from "semantic-ui-react";
  ...
  const renderInputs = () => {
    return inputFunction.map((result, index) => {
      if (result.tab === tab) {
        if (result.type === "dropDown") {
        ...
        else if(result.type==='optionlist'){
          const filter = optionData.filter((value) => value.type === result.name);
            
            const commodityOption=[
              {value:'Cars and Car Parts',text:'Cars and Car Parts'},
              {value:'Household Goods',text:'Household Goods'}
            ]
            ...
            return(
              <div style={{ padding: 10 }} key={index}>
                ...
                <Dropdown
                  ...
                  name={result.name}
                  onChange={(e, { value }) => {
                    handleInput(index, value as string)
                  }}
                />
          ...
          return (
            <div style={{ padding: 10 }} key={index}>
              ...
              <br />
              <Input type={result.type} name={result.name} value={result.value} onChange={(e, { value }) => handleInput(index, value)} />
            </div>
          );
        }
      }
