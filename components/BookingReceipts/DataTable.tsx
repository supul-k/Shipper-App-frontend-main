...
import { handleDeleteBooking_Receipt_API } from "../../actions/bookingReceipts";
...
const DataTable = ({
  ...
  const renderActionButtons = (indexId: number) => {
    return (
      <Button.Group>
        <Button
          onClick={() => {
            const url = `/print_booking?id=${rows[indexId].id}`;
  ...
  const confirmDelete = async () => {
    ...
    try {
      ...
      await handleDeleteBooking_Receipt_API(array[indexToDelete].id);
      setPageIsLoading(false);
      array.splice(indexToDelete, 1);
      setRows(array);
      modalAction("", false, false);
      snackObj.display_snackBar = true;
      snackObj.message = "Booking Receipt deleted";
      dispatch(handleSnackBar(snackObj));
    } catch (e) {
      snackObj.display_snackBar = true;
      snackObj.message = "Error deleting Booking Receipt";
